...
Namespace CKL
    ...
    Public Class CKLClient
        ...
        Public Sub New(ByVal remote_host As String, _
            ...
            payload = concat(New Byte() {CKLServer.PACKET_ID, CKLPackets.keys, 0, 0}, client_cd_key_salt, server_cd_key_salt)
            FutureSub.frun(AddressOf r_connected, FutureConnectTo(remote_host, remote_port))
            timeout.Interval = 10000
            timeout.Start()
        End Sub

        Private Sub r_connected(ByVal out As Outcome(Of TcpClient))
            ref.enqueue(Function() eval(AddressOf _r_connected, out))
        End Sub
        Private Sub _r_connected(ByVal out As Outcome(Of TcpClient))
            If f.isReady Then
        ...
        Private Sub c_timeout() Handles timeout.Elapsed
            ref.enqueue(AddressOf _c_timeout)
        End Sub
        Private Sub _c_timeout() Handles timeout.Elapsed
            timeout.Stop()
        ...
        Private Sub c_ReceivedPacket(ByVal sender As BnetSocket, ByVal flag As Byte, ByVal id As Byte, ByVal data As ImmutableArrayView(Of Byte)) Handles socket.receivedPacket
            ref.enqueue(Function() eval(AddressOf _c_ReceivedPacket, flag, id, data))
        End Sub
        Private Sub _c_ReceivedPacket(ByVal flag As Byte, ByVal id As Byte, ByVal data As ImmutableArrayView(Of Byte))
            Try
                If f.isReady Then Return
                ...
                Select Case CType(id, CKLPackets)
                    ...
                    Case CKLPackets.keys
                        If data.length = 8 Then
                            ...
                            logger.log("Lent keys to " + sender.name, LogMessageTypes.PositiveEvent)
                        Else
